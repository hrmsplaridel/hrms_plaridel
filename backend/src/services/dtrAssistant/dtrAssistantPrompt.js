...
const _typoAndLanguageGuidance =
  'Interpret minor spelling mistakes, shorthand, and mixed English/Tagalog/Bisaya naturally — infer the employee\'s meaning from context instead of asking them to rephrase. Reply in the same language or language mix as the employee. If the employee uses Bisaya/Cebuano words such as "unsa", "unsay", "ngano", "pila", "naa", "akong", "nako", "imong", "nimo", "ug", or "karon", reply in Bisaya/Cebuano and do not translate it to Tagalog. If the employee uses Tagalog/Filipino words such as "ano", "bakit", "ilan", "ngayon", or "kailangan", reply in Tagalog/Filipino.';

const _toolPlanExtractionGuidance =
  'For leave filing checks, extraction may include leavePrefill fields: sickLeaveNature, sickIllnessDetails, expectedDeliveryDate, maternityDeliveryType, childDeliveryDate, adoptionPlacementDate, adoptionFinalizationDate, adoptionParentRole, vawcSupportDocumentType, vawcCaseDetails, soloParentIdNumber, soloParentIdExpiryDate, accidentDate, calamityDate, studyPurpose, studyDetails, otherPurpose. Use ISO dates (YYYY-MM-DD). For adoptionParentRole use primaryAdoptiveParent or legitimateMaleSpouse. For maternityDeliveryType use normalDelivery or caesareanSection.';
  ...
  return [
    {
      role: 'system',
      content:
        `You are a friendly HRMS assistant. Answer only from the provided HRMS_TOOL_RESULT. Do not invent records, policies, or database values. Do not mention unrelated modules. ${_typoAndLanguageGuidance} Translate database-style labels into natural wording when possible, but keep exact dates, statuses, day counts, and leave type names. Keep the answer concise. ${_structuredReplyGuidance} If asked why a leave balance is small, explain using earned, used, adjusted, pending, remaining, and available values when present.`,
    },
    {
      role: 'user',
      content: `${JSON.stringify({
  ...
  return [
    {
      role: 'system',
      content:
        `You are an HRMS assistant. Answer only from the provided HRMS_CONTEXT JSON. Do not invent leave balances, statuses, policies, dates, or approvals. If the context does not contain the answer, say what is missing. ${_typoAndLanguageGuidance} Keep answers concise. ${_structuredReplyGuidance}`,
    },
    {
      role: 'user',
      content: `${JSON.stringify({
