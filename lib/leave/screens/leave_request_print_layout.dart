...
class LeaveRequestPrintLayout extends StatefulWidget {
  const LeaveRequestPrintLayout({
    super.key,
    this.initialRequest,
    this.onSaveDraft,
  ...
  @override
  State<LeaveRequestPrintLayout> createState() =>
      _LeaveRequestPrintLayoutState();
}

class _LeaveRequestPrintLayoutState extends State<LeaveRequestPrintLayout> {
  ...
  void initState() {
    ...
    final effectiveName = (initial?.employeeName ?? '').trim().isNotEmpty
        ? initial!.employeeName!
        : authName;
    final nameParts = _parseNameToLastFirstMiddle(effectiveName);
  ...
  Future<void> _loadEmployeeBalances() async {
    ...
    try {
      ...
      final balances = await repository.getBalancesForUser(empId);
      if (mounted) {
        setState(() {
          _employeeBalances = balances;
        });
      }
    } catch (_) {
      // Best effort; stays as dashes if failed
    }
  }
  ...
  Future<void> _loadEmployeeAssignmentPrefill() async {
    ...
    try {
      final res = await ApiClient.instance.get<List<dynamic>>(
        '/api/assignments?employee_id=$targetUserId&status=Active',
      );
      final data = res.data;
                    ...
                    child: Container(
                      ...
                      child: isDesktop
                          ? Row(
                              ...
                              children: [
                                Expanded(child: _buildAttachmentSection()),
        ...
        children: [
          ...
          Table(
            ...
            children: [
              ...
              TableRow(
                children: [
                  const _TableCell(text: 'Less this application'),
                  _TableCell(text: formatDays(vlDeduction)),
                  _TableCell(text: formatDays(slDeduction)),
                ],
              ),
              TableRow(
                children: [
  ...
  Future<void> _saveDraft() async {
    ...
    try {
      ...
      if (ok) {
        ...
        _showMessage(
          'Leave request draft saved. You can add an attachment if needed.',
        );
      } else {
  ...
  bool _validateForm() {
    ...
    if (_leaveType == LeaveType.others &&
      ...
      _showMessage(
        'Monetization and Terminal Leave are HR/admin processes. Please contact HR.',
      );
      return false;
    }
  ...
  LeaveRequest? _buildRequest({required LeaveRequestStatus status}) {
    ...
    return LeaveRequest(
      ...
      otherPurposeDetails: _leaveType == LeaveType.others
          ...
          : null,
      attachmentPath:
          current?.attachmentPath ?? widget.initialRequest?.attachmentPath,
      attachmentName:
          current?.attachmentName ?? widget.initialRequest?.attachmentName,
