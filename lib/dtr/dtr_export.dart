...
class DtrExport {
  ...
  static pw.Widget _buildFooter(double equivalentDay) {
    return pw.Column(
      ...
      children: [
        ...
        pw.Text('Verified as to the prescribed office hours.', style: const pw.TextStyle(fontSize: 7), textAlign: pw.TextAlign.center),
        pw.SizedBox(height: 12),
        pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
          pw.Text('HON. GADWIN E. HANDUMON', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
          pw.Text('Municipal Mayor', style: const pw.TextStyle(fontSize: 7)),
        ]),
        pw.SizedBox(height: 12),
        pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
          pw.Text('MARCELO B. CAÑARES', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
          pw.Text('Human Resource Mgt. and Dev\'t. Officer', style: const pw.TextStyle(fontSize: 7)),
    ...
    doc.addPage(
      pw.Page(
        ...
        build: (context) => pw.Column(
          ...
          children: [
            pw.Text('DAILY TIME RECORD', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.center),
            pw.SizedBox(height: 6),
            pw.RichText(
              textAlign: pw.TextAlign.center,
              text: pw.TextSpan(
                children: [
                  pw.TextSpan(text: 'Name : ', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
                  pw.TextSpan(text: employeeName.toUpperCase(), style: const pw.TextStyle(fontSize: 9)),
                ],
              ),
            ),
            ...
            pw.RichText(
              ...
              text: pw.TextSpan(
                children: [
                  pw.TextSpan(text: 'PERIOD : ', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
                  pw.TextSpan(text: '${_months[month - 1]} 1-${end.day}, $year', style: const pw.TextStyle(fontSize: 9)),
                ],
              ),
            ),
            pw.SizedBox(height: 2),
            pw.RichText(
              ...
              text: pw.TextSpan(
                children: [
                  pw.TextSpan(text: 'Official Hours: ', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
                  pw.TextSpan(text: '8:00AM-12:00PM 01:00PM-5:00PM', style: const pw.TextStyle(fontSize: 8)),
                ],
              ),
            ),
            pw.SizedBox(height: 6),
  ...
  static pw.Widget _cell(String text, double fs, {bool bold = false, bool center = false, bool right = false}) {
    pw.TextAlign? align;
    ...
    return pw.Padding(
      ...
      child: pw.Text(
        text,
        style: pw.TextStyle(fontSize: fs, fontWeight: bold ? pw.FontWeight.bold : null),
        textAlign: align,
      ),
    );
  }
  ...
  static Future<Uint8List> generateExcel({
    ...
    final greyBg = ExcelColor.fromHexString('FFD3D3D3'); // Light grey like reference DTR
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).cellStyle = CellStyle(fontSize: 11, bold: true, horizontalAlign: HorizontalAlign.Center);
    row++;
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).value = TextCellValue('Name : ${employeeName.toUpperCase()}');
    sheet.merge(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row), CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row));
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).cellStyle = CellStyle(fontSize: 9, horizontalAlign: HorizontalAlign.Center);
    row++;
    ...
    for (var d = 1; d <= end.day; d++) {
      ...
      for (var c = 1; c <= 4; c++) {
        sheet.cell(CellIndex.indexByColumnRow(columnIndex: c, rowIndex: row)).cellStyle = cellStyleCenter;
      }
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: row)).cellStyle = cellStyleRight;
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row)).cellStyle = cellStyleRight;
      row++;
    }
    ...
    for (var c = 0; c <= 4; c++) {
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: c, rowIndex: row)).cellStyle = totalStyle;
    }
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: row)).cellStyle = totalStyleRight;
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row)).cellStyle = totalStyleRight;
    row += 2;

    final equivalentDay = _computeEquivalentDay(totalUndertimeMin);
    final footerCenterStyle = CellStyle(fontSize: 9, horizontalAlign: HorizontalAlign.Center);
    ...
    sheet.merge(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row), CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row));
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).cellStyle = footerCenterStyle;
    row++;
    ...
    sheet.merge(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row), CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row));
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).cellStyle = CellStyle(fontSize: 9, bold: true, horizontalAlign: HorizontalAlign.Center);
    row++;
  ...
  static String generateWordHtml({
    ...
    sb.writeln('<style>@page{size:letter;margin:0.4in;}body{font-family:Arial,sans-serif;font-size:7pt;margin:12px;line-height:1.2;}');
    sb.writeln('.dtr-table{border-collapse:collapse;width:100%;font-size:6.5pt;}');
    sb.writeln('.dtr-table th,.dtr-table td{border:1px solid #333;padding:2px 4px;}');
    sb.writeln('.dtr-table th{background:#d3d3d3;font-weight:bold;text-align:center;}');
    ...
    sb.writeln('h2{margin:4px 0 8px;font-size:11pt;text-align:center;font-weight:bold;}</style></head><body>');
    sb.writeln('<h2>DAILY TIME RECORD</h2>');
    sb.writeln('<div style="text-align:center;font-size:10pt;line-height:1.4;">');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;"><strong>Name :</strong> ${employeeName.toUpperCase()}</p>');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;"><strong>PERIOD :</strong> ${_months[month - 1]} 1-${end.day}, $year</p>');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;"><strong>Official Hours:</strong> 8:00AM-12:00PM 01:00PM-5:00PM</p>');
    sb.writeln('</div>');
    sb.writeln('<table class="dtr-table">');
    sb.writeln('<thead><tr><th rowspan="2">Date</th><th colspan="2">AM</th><th colspan="2">PM</th><th colspan="2">UNDERTIME</th></tr>');
    sb.writeln('<tr><th>IN</th><th>OUT</th><th>IN</th><th>OUT</th><th>Hours</th><th>Min</th></tr></thead><tbody>');
    ...
    for (var d = 1; d <= end.day; d++) {
      ...
      sb.writeln('<tr><td>${_formatDateWithDay(dt)}</td><td class="center">$amIn</td><td class="center">$amOut</td><td class="center">$pmIn</td><td class="center">$pmOut</td><td class="right">${isWeekend ? '' : uh}</td><td class="right">${isWeekend ? '' : um}</td></tr>');
    }

    final totalH = totalUndertimeMin ~/ 60;
    final totalM = totalUndertimeMin % 60;
    sb.writeln('<tr class="total-row"><td>Total Undertime</td><td></td><td></td><td></td><td></td><td class="right">$totalH</td><td class="right">$totalM</td></tr>');
    sb.writeln('</tbody></table>');

    final equivalentDay = _computeEquivalentDay(totalUndertimeMin);
    sb.writeln('<div style="text-align:center;font-size:10pt;margin-top:12px;line-height:1.4;">');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;">Equivalent Day (deduction, 8 hr/day): ${equivalentDay.toStringAsFixed(3)}</p>');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;">I certify on my honor that the above is a true and correct report of the hours of work performed, record of which was made daily at the time of arrival and departure from office.</p>');
    sb.writeln('<p style="text-align:center;font-size:10pt;margin:4px 0;">Verified as to the prescribed office hours.</p>');
    sb.writeln('</div>');
    sb.writeln('<div style="text-align:center;margin-top:16px;font-size:10pt;line-height:1.4;">');
    sb.writeln('<p style="margin:0 0 16px;"><strong>HON. GADWIN E. HANDUMON</strong><br>Municipal Mayor</p>');
    sb.writeln('<p style="margin:0;"><strong>MARCELO B. CAÑARES</strong><br>Human Resource Mgt. and Dev\'t. Officer<br>01/13 ON FIELD</p>');
    sb.writeln('</div>');
