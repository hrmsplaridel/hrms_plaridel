...
import 'package:hrms_plaridel/api/client.dart';
import 'package:hrms_plaridel/docutracker/screens/docutracker_permission_editor_screen.dart';
...
void main() {
        ...
        InterceptorsWrapper(
          onRequest: (options, handler) {
            ...
            if (options.path.startsWith('/api/employees/')) {
              ...
              handler.resolve(
                Response(
                  ...
                  data: {
                    'id': userId,
                    'full_name': userId == 'u-hr' ? 'Beatriz Reviewer' : 'Unknown User',
                    'current_department_name': userId == 'u-hr' ? 'HR' : 'Unknown',
                  },
                ),
              );
              return;
            ...
            if (options.path == '/api/docutracker/permission-explain') {
              final action = (options.queryParameters['action']?.toString() ?? '').trim();
              final granted = action == 'view' || action == 'create_draft' || action == 'download';
              handler.resolve(
                Response(
                  requestOptions: options,
                  statusCode: 200,
            ...
            handler.resolve(
              Response(requestOptions: options, statusCode: 200, data: const <String, dynamic>{}),
            );
          },
        ),
      );
  ...
  testWidgets('renders 3 permission tabs and effective preview guidance', (tester) async {
  ...
  testWidgets('user search keeps selected employee visible in dropdown', (tester) async {
    await pumpEditor(tester, userTab: true);

    final searchField = find.widgetWithText(TextField, 'Search user (name, department, id)');
