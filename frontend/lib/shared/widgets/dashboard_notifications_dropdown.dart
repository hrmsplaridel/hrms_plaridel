...
class _DocuTrackerNotificationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final n = notification;
    final unread = !n.read;
    final title = n.title?.trim().isNotEmpty == true ? n.title! : n.displayType;
          ...
          decoration: BoxDecoration(
            color: unread
                ? (dark
                      ...
                      : DocuTrackerTokens.highlightPeach.withValues(
                          alpha: 0.65,
                        ))
                : (dark
                      ? const Color(0xFF111827)
...
class _DropdownHeader extends StatelessWidget {
  const _DropdownHeader({required this.unreadCount, this.onMarkAllRead});
