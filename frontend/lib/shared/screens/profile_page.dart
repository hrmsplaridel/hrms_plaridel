...
class _ProfileContentState extends State<ProfileContent> {
  ...
  void _applyUserProfile(AppUser u) {
    ...
    _nationalityValue = (u.nationality ?? '').trim().isEmpty
        ? null
        : u.nationality!.trim();
    ...
    try {
      final res = await ApiClient.instance.patch<Map<String, dynamic>>(
        ...
        data: {
          ...
          'last_name': last,
          'suffix': _suffixValue,
          'date_of_birth': _birthdateValue != null
  ...
  Widget _buildNationalityField(
    ...
    return Autocomplete<String>(
      ...
      fieldViewBuilder: (context, controller, focusNode, _) {
        if ((_nationalityValue ?? '').isNotEmpty && controller.text.isEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    return Selector<
      AuthProvider,
      ({String email, AppUser? user, String displayName, String? avatarPath})
    >(
      selector: (_, a) => (
        email: a.email,
        user: a.user,
      ...
      builder: (context, auth, _) {
        return LayoutBuilder(
          builder: (context, constraints) {
            final isWide = constraints.maxWidth >= _profileWideBreakpoint;
