...
class _ManageOvertimeState extends State<ManageOvertime> {
  ...
  Future<void> _submitRequest() async {
    ...
    if (otDate == null || timeStart == null || timeEnd == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please set OT date, start time, and end time.'),
        ),
      );
      return;
    }
    ...
    final timeStartStr =
        '${timeStart.hour.toString().padLeft(2, '0')}:${timeStart.minute.toString().padLeft(2, '0')}:00';
    final timeEndStr =
        '${timeEnd.hour.toString().padLeft(2, '0')}:${timeEnd.minute.toString().padLeft(2, '0')}:00';
    try {
      await ApiClient.instance.post(
        '/api/overtime',
        data: {
          if (_selectedEmployeeIdForSubmit != null)
            'employee_id': _selectedEmployeeIdForSubmit,
  ...
  Future<void> _review(String id, String status, [String? reviewNotes]) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Overtime $status.')));
        _loadRequests();
      }
    } on DioException catch (e) {
      if (mounted) {
  ...
  Future<void> _markAddedToPayroll(String id) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Marked as added to payroll.')),
        );
        _loadRequests();
      }
    } on DioException catch (e) {
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '${r.employeeName} · ${r.otDate.year}-${r.otDate.month.toString().padLeft(2, '0')}-${r.otDate.day.toString().padLeft(2, '0')}',
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_employees.isNotEmpty) ...[
            Text(
              'Employee (optional – leave blank for yourself)',
              style: TextStyle(fontSize: 12, color: AppTheme.textSecondary),
            ),
            const SizedBox(height: 6),
            DropdownButtonFormField<String>(
              initialValue: _selectedEmployeeIdForSubmit,
              ...
              onChanged: (v) =>
                  setState(() => _selectedEmployeeIdForSubmit = v),
            ),
            const SizedBox(height: 16),
          ],
          Text(
